            ...
            for (let i = 0; i < basket.length; i++) {
                ...
                if (sellerCityProductFit.length == 0) {
                    ...
                    for (let j = 0; j < basket[i].product.user.cityWay.length; j++) {
                        ...
                        for (let v = 0; v < basket[i].product.user.cityWay[j].cityWayTransport.length; v++) {
                            if (
                                ...
                                basket[i].product.user.sellerSity.find(
                                    (item: any) => item.idCity == cityUser?.city?.id && item.typePoint == "PickPoint"
                                )
                            ) {
                    ...
                    if (isDeliveryProduct) {
                        ...
                        for (let w = 0; w < basket[i].product.user.cityWay.length; w++) {
                            ...
                            if (masDuration.length > 0) {
                                graph[basket[i].product.user.cityWay[w].city1.name][basket[i].product.user.cityWay[w].city2.name] = Math.min(
                                    ...masDuration
                                );
                                graph[basket[i].product.user.cityWay[w].city2.name][basket[i].product.user.cityWay[w].city1.name] = Math.min(
                                    ...masDuration
                                );

                                graph2[basket[i].product.user.cityWay[w].city1.name][basket[i].product.user.cityWay[w].city2.name] = Math.min(
                                    ...masCost
                                );
                                graph2[basket[i].product.user.cityWay[w].city2.name][basket[i].product.user.cityWay[w].city1.name] = Math.min(
                                    ...masCost
                                );

                                graph3[basket[i].product.user.cityWay[w].city1.name][basket[i].product.user.cityWay[w].city2.name] = Math.min(
                                    ...masLengh
                                );
                                graph3[basket[i].product.user.cityWay[w].city2.name][basket[i].product.user.cityWay[w].city1.name] = Math.min(
                                    ...masLengh
                                );
                            }
                        }
                        ...
                        for (let e = 0; e < sellerCityProductFit.length; e++) {
                            let paths = findPathsAndSums(
                                convertGraphFormat(graph),
                                sellerCityProductFit[e].sellerCity.city.name,
                                cityUser?.city?.name
                            );
                            let paths2 = findPathsAndSums(
                                convertGraphFormat(graph2),
                                sellerCityProductFit[e].sellerCity.city.name,
                                cityUser?.city?.name
                            );
                            let paths3 = findPathsAndSums(
                                convertGraphFormat(graph3),
                                sellerCityProductFit[e].sellerCity.city.name,
                                cityUser?.city?.name
                            );
                        ...
                        for (let z = 0; z < mainGraph.length; z++) {
                            ...
                            mainResult.push({
                                ...
                                indexBestPathX,
                                costMin: tempAllCost,
                                durationMin: tempAllDuration,
                                lenghtMin: tempAllLenght,
                            });
                        }
                        ...
                        for (let t = 0; t < mainResult.length; t++) {
                            if (
                                data.maxDuration >= mainResult[t].durationMin &&
                    ...
                    if (isDeliveryProduct) {
                        ...
                        for (let z = 0; z < mainGraph.length; z++) {
                            if (
                                data.maxDuration >= mainGraph[z]["parameters"][0] &&
                                ...
                                if (mainResultBest.length == 0) {
                                    ...
                                    mainResultBest["parameters"][0] > mainGraph[z]["parameters"][0] &&
